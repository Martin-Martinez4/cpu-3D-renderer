...
void triangle_from_polygon(polygon_t* polygon, triangle_t triangles[], int* num_triangles){
...
void clip_polygon_against_plane(polygon_t* polygon, int plane){
	...
	tex2_t* current_texcoord = &polygon->textcoords[0];
	tex2_t* previous_texcoord = &polygon->textcoords[polygon->num_vertices - 1];
	...
	for(int i = 0; i < num_inside_vertices; i++){
		...
		polygon->textcoords[i] = tex2_clone(&inside_texcoords[i]);
	}
